...
describe('prettyprint', function() {
  ...
  it ('should substitute scope variables', function () {
      ...
      var res = math.prettyprint(funcStr, scope)
      assert.equal(res, "(0.00) + (2.13e+6)*x - (-212)*x^2 + (0.00)*x^3 + (1.00e-19)*x^4")

      var funcStr = 'a + h*e^( -(x-x0)^2/(2*w^2) )';
      var scope = {a : 0.0, h : 1, x0 : -212.22, w : 1};
      var res = math.prettyprint(funcStr, scope)
      assert.equal(res, "(0.00) + (1.00)*e^((-(x - (-212))^2) / (2*(1.00)^2))")

      var funcStr = 'a + b*cos(2*c*x) + c*sin(x)';
      var scope = {a : 0.0, b : 1, c : 1.321213};
      var res = math.prettyprint(funcStr, scope)
      assert.equal(res, "(0.00) + (1.00)*cos(2*(1.32)*x) + (1.32)*sin(x)")
