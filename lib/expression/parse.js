 ...
 * @private
 */
function nextPreview(i) {
  return expression.charAt(index + i);
}
...
function getToken() {
  ...
  if (isDigitDot(c)) {
    ...
    if (c == 'E' || c == 'e') {
      ...
      // check if it is exponent function 'exp' and not scientific notation
      var c3 = c + nextPreview(1) + nextPreview(2);
      if (c3 == 'exp') {
        next();
        next();
        return;
