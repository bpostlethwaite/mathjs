module.exports = function (math) {
  ...
  math.prettyprint = function (expr, scope, options) {
    ...
    options.doSimplify = options.doSimplify || false;
    options.doTex = options.doTex || false;
  
    if (isString(expr)) {
      // prettyprint a single expression
      var node = math.parse(expr);
        
      // patch .toString() with scope substitution 
      enableScope(node, scope, options.precision);

      if (options.doSimplify) {
        // patch .toString(), adding simplification algorithms
        enableSimplify(node);
      }
    
      if (options.doTex) {
        // call .toString, parse and call toTex() 
        return doTex(node);
      }
      else {
  ...
  function enableScope (node, scope, precision) {
    /**
     * Over-ride Object.prototype.toString
     * method with an Object.toString method
     * that enables scope substitution
     */
  ...
  function enableSimplify (node) {
    /**
     * Over-ride Object.prototype.toString
     * method with an Object.toString method
     * that enables mathematical simplification
     */
  ...
  function doTex (node) {
    /**
     * Call .toString(), parse and call toTex()
     * to generate Tex consistent with enableSimplify()
     *
     * - mathjs can't parse e.g. '121e^x' -> replace to '121 e^x'
     * - mathjs can't parse '+/-' -> replace to a dummy 'mod'
     *   which is then replace to '\pm' after .toTex()
     */
    var node_str = node.toString()
                       .replace(/exp/g, ' e^')
                       .replace(/e\^/g, ' e^')
                       .replace(/\+\/\-/g, 'mod');
    return math.parse(node_str)
                .toTex()
                .replace(/\\bmod/g, '\\pm');
  };
