module.exports = function (math) {
  ...
  function operatorNodetoString (precision) {
    /*
    ...
    switch (params.length) {
       ...
       * sin(x) * 0 -> "0"
       */
      if (lhs === (1).toPrecision(precision) && op === "*")
        return rhs;

      if (rhs === (1).toPrecision(precision) && op === "*")
        return lhs;

      if (lhs === (0).toPrecision(precision))
        return (precedence(this) === 8) ? rhs : "0";

      if (rhs === (0).toPrecision(precision))
  ...
  function enablePretty (node, scope, precision) {
    ...
    operators.forEach( function (o) {
      o.toString = operatorNodetoString(precision)
    });
